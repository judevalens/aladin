...
// Resolver maps free-text entity mentions to canonical entity ids. R0 implements the
// foundation: normalize → exact-key lookup in the shared tier → resolve-or-create,
// writing an alias + a mention (both idempotent). Fuzzy matching, proposed merges,
// embeddings, the tenant tier, and same-name/same-kind splitting are later phases.
