...
# --- native prod ports (must not collide with the dev loop: api 8000, mcp 8090,
#     blocknote 3500/3501). These match what the containers published, so the
#     desktop app built by 'make prod-app' needs no change.
API_ADDR=":8080"
MCP_HTTP_ADDR=":8091"
PORT="3510"
COLLAB_PORT="3511"
...
COPILOT_AGENT_URL="http://127.0.0.1:3550"
...
emit_run blocknote      "\"$NODE_BIN\" \"\$HERE/services/blocknote/server.js\""
emit_run copilot-agent  "\"$NODE_BIN\" \"\$HERE/services/copilot-agent/server.js\""
...
if [[ -z "${NO_SWITCH:-}" ]]; then
  ln -sfn "$DEST" "$PREFIX/current.new" && mv -f "$PREFIX/current.new" "$PREFIX/current"
  say "current -> $STAMP"
else
