...
>(({ className, inset, ...props }, ref) => (
  <DropdownMenuPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex cursor-default select-none items-center rounded px-2.5 py-1.5 text-[13px] outline-none transition-colors focus:bg-raise data-[highlighted]:bg-raise data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
      inset && "pl-8",
      className,
    )}
    {...props}
...
export function DropdownMenuLabel({ className, inset, ...props }: React.ComponentPropsWithoutRef<typeof DropdownMenuPrimitive.Label> & { inset?: boolean }) {
  return (
    <DropdownMenuPrimitive.Label
      className={cn("px-2.5 py-1 font-mono text-[10.5px] font-semibold uppercase tracking-[0.08em] text-ink-3", inset && "pl-8", className)}
      {...props}
    />
  );
}
...
>(({ className, inset, children, ...props }, ref) => (
  <DropdownMenuPrimitive.SubTrigger
    ref={ref}
    className={cn(
      "flex cursor-default select-none items-center px-3 py-2 text-sm outline-none focus:bg-raise data-[highlighted]:bg-raise",
      inset && "pl-8",
      className,
    )}
    {...props}
...
>(({ className, ...props }, ref) => (
  <DropdownMenuPrimitive.SubContent
    ref={ref}
    className={cn("z-50 min-w-[12rem] rounded-md border border-line bg-explorer p-1 shadow-modal", className)}
    {...props}
  />
));
...
>(({ className, children, checked, ...props }, ref) => (
  <DropdownMenuPrimitive.CheckboxItem
    ref={ref}
    className={cn("relative flex select-none items-center py-2 pl-8 pr-3 text-sm outline-none focus:bg-raise data-[highlighted]:bg-raise", className)}
    checked={checked}
    {...props}
  >
