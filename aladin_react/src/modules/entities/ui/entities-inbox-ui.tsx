...
function EntityRef({ name, kind }: { name: string; kind: string }) {
  return (
    <span className="inline-flex min-w-0 items-baseline gap-1.5">
      <span className="truncate font-display text-lead font-semibold text-ink">{name}</span>
      <span className="shrink-0 font-mono text-meta text-ink-4">{kind}</span>
    </span>
  );
}
...
export function QueueCard({
  ...
  return (
    <div className="rounded-card border border-line-2 bg-card p-3.5">
      ...
      <div className="mt-1.5 flex items-center gap-2 font-mono text-meta">
      ...
      <div className="mt-3 flex items-center gap-2">
        <button
          ...
          className="flex cursor-pointer items-center gap-1 rounded-chip bg-amber px-2.5 py-1 text-meta font-semibold text-primary-foreground transition hover:brightness-[1.08] disabled:opacity-40"
        >
        ...
        <button
          ...
          className="flex cursor-pointer items-center gap-1 rounded-chip border border-line px-2.5 py-1 text-meta font-semibold text-ink-2 transition hover:brightness-[1.08] disabled:opacity-40"
        >
        ...
        <button
          type="button"
          onClick={onOpen}
          className="ml-auto cursor-pointer font-mono text-meta text-ink-4 hover:text-ink-2"
        >
          view →
        </button>
      </div>
  ...
  return (
    <div className="mx-auto max-w-[620px] px-5 py-5">
      <div className="mb-4 flex items-baseline gap-2.5">
        <span className="font-display text-lead font-semibold text-ink">Same, or just similar?</span>
      ...
      {error ? (
        ...
        <div className="flex flex-col items-center gap-2 rounded-card border border-line-2 bg-card px-5 py-10 text-center">
          {/* Empty-state illustration at 22px — §5 rule 9's carve-out. */}
          <Inbox size={22} strokeWidth={1.6} className="text-ink-4" />
          <p className="text-body font-medium text-ink-2">Inbox zero</p>
          <p className="max-w-xs text-small leading-[1.5] text-ink-4">
