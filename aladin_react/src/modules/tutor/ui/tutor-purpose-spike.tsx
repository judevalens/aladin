  ...
  return (
    <div className="flex h-screen overflow-hidden bg-bg text-ink">
      ...
      <aside className="w-[280px] shrink-0 border-r border-line bg-panel py-3">
        <div className="flex items-center gap-1 px-3 py-1 text-small text-ink-3">
          <Icon as={ChevronDown} size="inline" mark className="text-ink-4" />
          Pairs trading
        </div>
        {TREE.map((f) => {
          const Glyph = GLYPH[f.purpose];
          return (
            <button
              key={f.id}
              type="button"
            ...
            >
              <Icon as={Glyph} size="inline" mark className="shrink-0 text-ink-4" />
              <span className="min-w-0 flex-1 truncate text-small text-ink-2">{f.title}</span>
  ...
  return (
    <div className="mx-auto w-full max-w-[62rem] px-8 py-7">
      ...
      <Section label={learning ? "What I want out of this" : "Hypothesis"}>
        <p className="font-display text-lead leading-relaxed text-ink">
          {learning
...
function Section({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <section className="mb-8">
      <Eyebrow as="h2" className="mb-2 text-ink-4">{label}</Eyebrow>
...
function Row({
  ...
  return (
    ...
    >
      {Glyph ? <Icon as={Glyph} size="inline" mark className="mt-0.5 shrink-0 text-ink-4" /> : null}
      <span className="min-w-0 flex-1 truncate text-body text-ink-2">{title}</span>
      {mid ? <span className="shrink-0 font-mono text-meta text-ink-4">{mid}</span> : null}
      <span className="shrink-0 font-mono text-meta text-ink-4">{right}</span>
    </button>
  );
}
