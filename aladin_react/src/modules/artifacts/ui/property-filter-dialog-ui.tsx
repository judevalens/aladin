      ...
      <DialogContent className="w-[min(94vw,760px)] p-0">
        ...
        <div className="flex h-[min(70vh,520px)]">
          ...
          <div className="w-[240px] shrink-0 overflow-y-auto border-r border-line p-2">
            {facetsLoading ? (
              <p className="px-2 py-4 text-body text-ink-4">Loading…</p>
            ) : facets.length === 0 ? (
              <p className="px-2 py-4 text-body text-ink-4">
              ...
              facets.map((f) => (
                <div key={f.key} className="mb-2">
                  <button
                    ...
                    className={cn(
                      "flex w-full items-center gap-2 rounded-chip px-2 py-1.5 text-left text-body transition-colors",
                      key === f.key && !value ? "bg-raise text-ink" : "text-ink-2 hover:bg-card",
                    )}
                  >
                    <Icon as={Filter} size="inline" className="text-ink-4" />
                    <span className="flex-1 truncate font-display font-semibold">{f.key}</span>
                    <span className="font-mono text-meta text-ink-4">any</span>
                  </button>
                  <div className="ml-4 mt-0.5 flex flex-wrap gap-1">
                    {f.values.map((v) => (
                      <button
                        ...
                        className={cn(
                          "rounded-chip border px-1.5 py-0.5 text-meta transition-colors",
                          key === f.key && value === v
          ...
          <div className="flex min-w-0 flex-1 flex-col">
            <div className="flex shrink-0 items-center gap-2 border-b border-line px-3 py-2 text-small">
              {key ? (
                <>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-2">
              {!key ? null : results === undefined ? (
                <p className="px-2 py-8 text-center text-body text-ink-4">Searching…</p>
              ) : results.length === 0 ? (
                <p className="px-2 py-8 text-center text-body text-ink-4">No artifacts match.</p>
              ) : (
                results.map((a) => {
                  const Glyph = KIND_ICONS[a.kind] ?? FileText;
                  return (
                    <button
                      key={a.id}
                      type="button"
                    ...
                    >
                      <Icon as={Glyph} size="inline" className="shrink-0 text-ink-4" />
                      <span className="flex-1 truncate text-body text-ink">{a.title}</span>
                    </button>
                  );
                })
              )}
