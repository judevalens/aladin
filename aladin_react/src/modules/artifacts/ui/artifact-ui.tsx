      ...
      <section className="space-y-4 border-b border-line pb-6">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 space-y-1.5">
            <div className="eyebrow">Source{host ? ` · ${host}` : ""}</div>
        ...
        {artifact.sourceUrl ? (
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              <ExternalLink className="size-[15px]" strokeWidth={1.75} />
              Open link
            </Button>
            <a
              className="truncate text-[13px] text-amber underline-offset-4 hover:underline"
  ...
  return (
    <div className="mx-auto max-w-workspace-max px-8 py-8">
      <section className="space-y-4 border-b border-line pb-6">
        ...
        {loading ? (
          <p className="text-[13px] text-ink-3">Loading audio…</p>
        ) : error ? (
          <p className="text-[13px] text-against">Couldn’t load the audio. {error}</p>
        ) : url ? (
  ...
  return (
    <div className="mx-auto max-w-workspace-max px-8 py-8">
      <section className="space-y-4">
        ...
        {artifact.summary ? (
          <p className="text-[13.5px] leading-[1.65] text-ink-2">{artifact.summary}</p>
        ) : null}

        {loading ? (
          <p className="text-[13px] text-ink-3">Loading file…</p>
        ) : error ? (
          <p className="text-[13px] text-against">Couldn’t load the file. {error}</p>
          ...
          <iframe src={url} title={artifact.title} className="h-[75vh] w-full rounded-md border border-line" />
        ) : category === "video" ? (
          <video src={url} controls className="max-h-[70vh] w-full rounded-md border border-line" />
        ) : category === "audio" ? (
