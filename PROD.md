...
| Backup | `scripts/ops/backup_prod.sh` + launchd plist | Postgres + shard MongoDB + file root |
...
| Shard MongoDB | **27018** | 27017 / — |
| API | **8080** | 8000 |
| MCP | **8091** | 8090 |
| Shard resolver runtime | **8092** | 8092 |
...
`api,worker,mcp,collab,copilot`) selects which start. The data tier—Postgres,
Redis, Neo4j, and MongoDB—always starts. The optional `shards` profile runs the
resolver sidecar when using the all-Compose app tier; native releases add it
automatically when `SHARD_V2_ENABLED=1`.
...
| `shards` | shard-runtime | immutable GraphQL resolvers and manual lambdas |
...
# To enable Shard v2, change SHARD_V2_ENABLED=0 to 1 in backend_v2/.env.prod.
# The generated MongoDB URI and runtime secret are already complete.
...
**Restore every member of the set** — they share one timestamp for exactly this
reason. PostgreSQL alone omits shard-owned records and leaves document rows
...
docker compose -p aladin-prod --env-file backend_v2/.env.prod \
  -f docker-compose.prod.yml up -d postgres redis neo4j mongo mongo-init
# 1. the database
...
# 2. shard-owned MongoDB data, from the archive with the SAME timestamp
docker exec -i aladin-prod-shard-mongo mongorestore --drop --archive --gzip \
  < ~/aladin-backups/aladin-prod-YYYYMMDD-HHMMSS-mongo.archive.gz
# 3. the files, from the archive with the SAME timestamp
...
| tier | where | what |
|---|---|---|
| data | Docker (`docker-compose.prod.yml`) | postgres · shard MongoDB · redis · neo4j (+ loki/grafana) |
| app | native release | api · worker · mcp · blocknote · copilot-agent · shard-runtime when enabled |
