...
| **D-K** | **Where the strategy/learning discriminator lives** | **OPEN — do not implement.** A `purpose` column on `research_strategies` was proposed and **rejected**: it mixes table responsibilities, since a learning row would carry five strategy columns it can never use (`exec_mode`, `run_state`, `manifest`, `code_hash`, `universe`) in a table whose name says the rows are strategies. Options: **(B)** a neutral base row (discriminator + shared prose) with `research_strategies` and `learning_folders` each 1:1 off it — clean, costs moving existing columns; **(C)** `kind='learning'` with its own extension — cleanest, pays the ~31-file cost of a fourth kind; **(D)** extension-row presence as the discriminator — no new column, but "research" then names two things and reads need two joins. The **read model** is settled either way (see `/spike/tutor-purpose`): one discriminator drives the tree glyph, which Overview sections render, which views the tab group offers, and which system prompt the copilot uses — and must not fork the route, the tree, or the Overview component. |
